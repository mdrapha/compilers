...
selecao_decl
    : T_IF T_LPAREN expressao T_RPAREN statement else_decl {
        $$ = newNode();
        $$->type = T_IF; // Defining the type as an if statement
        strncpy($$->lexeme, "IF", MAX_LEXEME_SIZE - 1); // Set lexeme to "IF"
        $$->lexeme[MAX_LEXEME_SIZE - 1] = '\0'; // Ensure null termination
        $$->lineNumber = currentTokenLine;
        ...
        if ($6 != NULL) {
            addNode(&$$, $6, 2); // Add the else part as the third child, if it exists
        }
...
else_decl
    : T_ELSE statement {
        $$ = $2; // The result is the statement's subtree
    }
    | /* empty */ {
        $$ = NULL; // Representing an empty statement with NULL
    }
...
ativacao
    : fun_id ABREPARENTESES args FECHAPARENTESES {
        $$ = newNode();
        $$->nodeType = nAtivacao; 
        $$->type = EXPRESSAO; // Defining the type as an expression
        ...
        $$->expressionType = AtivK; // Defining the expression type as a function call
